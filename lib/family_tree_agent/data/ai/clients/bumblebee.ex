defmodule FamilyTreeAgent.AI.Clients.Bumblebee do
  ...
  def init(config \\ []) do
    ...
    with {:ok, embedding_model} <- load_embedding_model(embedding_model_name),
         ...
         {:ok, chat_model, chat_tokenizer, generation_config} <-
           load_chat_model(chat_model_name, config) do
      client = %__MODULE__{
  ...
  @impl true
  @spec info(FamilyTreeAgent.AI.RAGClient.t()) :: %{
          chat_model: any(),
          embedding_model: any(),
          provider: <<_::168>>
        }
  ...
  defp load_chat_model(model_name, config) do
    ...
    with {:ok, chat_model} <-
           Bumblebee.load_model({:hf, model_name, auth_token: huggingface_token}),
         {:ok, chat_tokenizer} <-
           Bumblebee.load_tokenizer({:hf, model_name, auth_token: huggingface_token}),
         {:ok, generation_config} <-
           Bumblebee.load_generation_config({:hf, model_name, auth_token: huggingface_token}) do
